...
export const deleteGradeFormulasForAssignment = async (sectionId, subjectId) => {
  // const { sectionId, subjectId } = req.params;

  try {
    // Validate section and subject IDs
    if (!isValidId(sectionId) || !isValidId(subjectId)) {
      return { message: 'Invalid section or subject ID' };
    }
    ...
    return { message: 'Grade formulas deleted successfully' };

  } catch (err) {
    console.error(err);
    return { message: 'Error deleting grade formulas' };
  }
};
